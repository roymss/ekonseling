                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 bg-white border border-[#C4C6CF] rounded-lg p-6 flex flex-col gap-6 relative overflow-hidden min-h-[298px]">
                        ...
                        <div class="bg-[#F9F9FF] border border-[#C4C6CF] rounded p-4 flex flex-col gap-3 z-10 h-full justify-center">
                            ...
                            <div class="flex justify-end gap-3 pt-2" x-data="{ showRescheduleModal: false }">
                                <button type="button" @click.prevent="showRescheduleModal = true" class="text-[#BA1A1A] text-[14px] font-medium hover:underline">Jadwal Ulang</button>

                                <!-- Reschedule Modal Popup -->
                                <div x-show="showRescheduleModal" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                                            ...
                                            <form action="{{ route('psikolog.konsultasi_reschedule', $sesi_mendatang->id_konsul) }}" method="POST">
                                                ...
                                                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-[#C4C6CF]">
                                                    ...
                                                    <div class="mt-4 space-y-4">
                                                        <div>
                                                            <label class="block text-sm font-semibold text-[#111C2C] mb-2">Tanggal Baru</label>
                                                            <input type="date" name="tanggal" class="w-full px-4 py-2 border border-[#C4C6CF] rounded-lg bg-[#F9F9FF] focus:outline-none focus:ring-1 focus:ring-[#002045] text-sm" value="{{ $sesi_mendatang->tanggal }}" required>
                                                        </div>
                                                        <div>
                                                            <label class="block text-sm font-semibold text-[#111C2C] mb-2">Jam Baru (WIB)</label>
                                                            <input type="time" name="jam" class="w-full px-4 py-2 border border-[#C4C6CF] rounded-lg bg-[#F9F9FF] focus:outline-none focus:ring-1 focus:ring-[#002045] text-sm" value="{{ $sesi_mendatang->jam }}" required>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse border-t border-[#C4C6CF]">
                                                    <button type="submit" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-[#002045] text-base font-medium text-white hover:bg-[#001530] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#002045] sm:ml-3 sm:w-auto sm:text-sm">
                                                        Simpan Jadwal Baru
                                                    </button>
                                                    <button type="button" @click="showRescheduleModal = false" class="mt-3 w-full inline-flex justify-center rounded-md border border-[#C4C6CF] shadow-sm px-4 py-2 bg-white text-base font-medium text-[#43474E] hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#002045] sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                                                        Batal
                                                    </button>
                                                </div>
                                            </form>
