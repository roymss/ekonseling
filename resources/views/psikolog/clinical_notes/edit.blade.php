...
<div class="w-full bg-[#F9F9FF] font-['Inter'] min-h-screen">
    <div class="max-w-[800px] mx-auto py-12 px-4 md:px-0">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-3xl font-bold text-[#111C2C]">Edit Catatan Klinis</h1>
            <p class="text-[#43474E] mt-1">Perbarui detail sesi konseling dan rekam medis pasien.</p>
        </div>
        ...
        <div class="bg-white border border-[#C4C6CF] rounded-xl shadow-sm overflow-hidden">
            <form action="{{ route('psikolog.clinical_notes.update', $note->id) }}" method="POST" class="p-6 md:p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8" x-data="{
                    ...
                    selectedUsername: '{{ old('patient_username', $note->patient_username) }}',
                    patientName: '{{ old('patient_name', $note->patient_name) }}',
                    patientBirthdate: '{{ old('patient_birthdate', $note->patient_birthdate) }}',
                    updatePatient() {
                    ...
                    <div>
                        ...
                        <input type="date" name="date" class="w-full px-4 py-3 border border-[#C4C6CF] rounded-xl bg-[#F9F9FF] text-[#111C2C] focus:outline-none focus:ring-1 focus:ring-[#002045] focus:border-[#002045] focus:bg-white transition-colors text-sm" value="{{ old('date', $note->date) }}" required>
                    </div>
                </div>
                ...
                <div class="space-y-6 mb-8">
                    <div>
                        ...
                        <textarea name="diagnosis" rows="4" class="w-full px-4 py-3 border border-[#C4C6CF] rounded-xl bg-[#F9F9FF] text-[#111C2C] focus:outline-none focus:ring-1 focus:ring-[#002045] focus:border-[#002045] focus:bg-white transition-colors text-sm" required>{{ old('diagnosis', $note->diagnosis) }}</textarea>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-semibold text-[#111C2C] mb-2">Treatment / Intervensi <span class="text-[#BA1A1A]">*</span></label>
                        <textarea name="treatment" rows="4" class="w-full px-4 py-3 border border-[#C4C6CF] rounded-xl bg-[#F9F9FF] text-[#111C2C] focus:outline-none focus:ring-1 focus:ring-[#002045] focus:border-[#002045] focus:bg-white transition-colors text-sm" required>{{ old('treatment', $note->treatment) }}</textarea>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                        ...
                        <input type="text" name="counselor_name" class="w-full px-4 py-3 border border-[#C4C6CF] rounded-xl bg-slate-100 text-[#43474E] text-sm" value="{{ old('counselor_name', $note->counselor_name) }}" required>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-semibold text-[#111C2C] mb-2">Lisensi / SIPK <span class="text-[#BA1A1A]">*</span></label>
                        <input type="text" name="counselor_license" class="w-full px-4 py-3 border border-[#C4C6CF] rounded-xl bg-[#F9F9FF] text-[#111C2C] focus:outline-none focus:ring-1 focus:ring-[#002045] focus:border-[#002045] focus:bg-white transition-colors text-sm" value="{{ old('counselor_license', $note->counselor_license) }}" required>
                    </div>
                </div>
                ...
                <div class="pt-6 border-t border-[#E1E6F3] flex justify-end gap-4">
                    ...
                    <button type="submit" class="px-6 py-3 bg-[#002045] text-white font-medium rounded-xl hover:bg-[#001530] shadow-sm transition-colors flex items-center">
                        <i class="fa-solid fa-floppy-disk mr-2"></i> Update Catatan
                    </button>
                </div>
            </form>
        </div>
