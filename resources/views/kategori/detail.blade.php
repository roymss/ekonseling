...
<div class="max-w-4xl mx-auto mb-12">
    ...
    <div class="text-sm text-gray-500 mb-6 flex flex-wrap items-center gap-2">
        ...
        <span>Kategori</span> 
        <span><i class="fa-solid fa-angle-right text-xs"></i></span> 
        <span class="text-gray-800 font-medium">{{ $rows->nama_kategori }}</span>
    </div>  
    
    <h2 class="text-3xl font-bold text-gray-900 mb-8 pb-3 border-b-2 border-green-600 inline-block">Kategori "{{ $rows->nama_kategori }}"</h2>
    
    <div class="space-y-8">
        @foreach ($beritakategori as $pi)
            @php
                $total_komentar = \Illuminate\Support\Facades\DB::table('komentar')->where(['id_berita' => $pi->id_berita, 'aktif' => 'Y'])->count();
    ...
    <div class="mt-10">
        {{ $beritakategori->links('pagination::tailwind') }}	
    </div>
</div>
@endsection
