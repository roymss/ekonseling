...
@php
	$baca = $rows->dibaca + 1;	
	$total_komentar = \Illuminate\Support\Facades\DB::table('komentar')->where(['id_berita' => $rows->id_berita, 'aktif'=>'Y'])->count();
@endphp	
<div class="max-w-4xl mx-auto mb-12">
    ...
    <div class="text-sm text-gray-500 mb-6 flex flex-wrap items-center gap-2">
        ...
        <a href="{{ url('kategori/detail/'.$rows->kategori_seo) }}" class="hover:text-green-600 transition-colors" rel="category tag">{{ $rows->nama_kategori }}</a> 
        <span><i class="fa-solid fa-angle-right text-xs"></i></span> 
    ...
    <header class="mb-8 border-b border-gray-200 pb-6">
        ...
        @if($rows->sub_judul)
        <div class="text-lg text-blue-600 font-medium mb-4">
            {{ $rows->sub_judul }}
        </div>
        @endif
    ...
    <div class="bg-gray-50 rounded-xl p-6 md:p-8">
        ...
        @if ($total_komentar >= 1)
            <div class="space-y-6 mb-10">
                ...
                if (session('level') != 'inovator') {
                    $komentar = \Illuminate\Support\Facades\DB::table('komentar')->where('id_berita', $rows->id_berita)->orderBy('id_komentar', 'ASC')->get();
                } else {
                    $komentar = \Illuminate\Support\Facades\DB::table('komentar')->where(['id_berita' => $rows->id_berita, 'aktif' => 'Y'])->orderBy('id_komentar', 'ASC')->get();
                }
                @endphp
        ...
        @if (session('level') != '')
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                ...
                <form method="POST" action="{{ url('berita/kirim_komentar') }}" class="space-y-4">
                    @csrf
                    ...
                    <div class="pt-2">
                        <button type="submit" name="submit" class="bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-6 rounded-md transition-colors shadow-sm" onclick="return confirm('Pesan anda akan tampil setelah kami setujui. Lanjutkan?')">
                            Kirim Komentar
                        </button>
                    </div>
                </form>
